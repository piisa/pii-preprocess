...
VERSION_FILE := src/pii_preprocess/__init__.py
...
backup:
	tar cvjf pii-preprocess-$(VERSION).tgz \
	--exclude=__pycache__ --exclude=pii_preprocess.egg-info \
	doc src test \
...
unit-full: venv pytest
	PYTHONPATH=src:test:../pii-data/src $(VENV)/bin/pytest -vv --capture=no $(ARGS) $(TEST)
